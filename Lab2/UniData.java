...
public class UniData implements Comparable<UniData> {
    public String rank2024;
    public String rank2023;
    ...
    public static UniData fromCSV(String csvLine) {
        ...
        for (int i = 10; i <= 28; i++) {
        try {
            double rawValue = Double.parseDouble(fields[i]);
            long roundedValue = Math.round(rawValue);
            fields[i] = String.valueOf(roundedValue);
    ...
    // It compares the name of the current university to the other university's name
        return this.institutionName.compareTo(other.getInstitutionName());}
}